  ...
  for (var i = 0; i < ROWS; ++i) {
    for (var j = 0; j < COLS; ++j) {
      ...
      while (stack.length) {
        ...
        if (n.row > 0) {
          stack.push(make_rc(n.row - 1, n.col));
        }
        if (n.row < ROWS - 1) {
          stack.push(make_rc(n.row + 1, n.col));
        }
        if (n.col > 0) {
          stack.push(make_rc(n.row, n.col - 1));
        }
        if (n.col < COLS - 1) {
          stack.push(make_rc(n.row, n.col + 1));
        }
...
function compute_heur(paramBoard) {
  ...
  if (globalheur == 5) {
    ...
    if (sum < 6) return 3 * sum + 1;
    if (sum == 6) return 49;
  }

  if (globalheur == 6) {
    //D/L Anubis
    if (all_matches.length < 8) return all_matches.length * 2;
    if (all_matches.length == 8) return 16;
    if (all_matches.length == 9) return 49;
    if (all_matches.length >= 10) return 100;
  }

  if (globalheur == 7) {
    // Bastet
    if (all_matches.length < 4) return all_matches.length;
    if (all_matches.length == 4) return 6.25;
    if (all_matches.length == 5) return 9;
    if (all_matches.length == 6) return 12.25;
    if (all_matches.length >= 7) return 16;
  }
...
function copy_solution(solution) {
  return copy_solution_with_cursor(
    solution,
    solution.cursor.row,
    solution.cursor.col,
...
function evolve_solutions(solutions, weights, dir_step) {
  ...
  solutions.sort(function (a, b) {
    return b.weight * b.heur - a.weight * a.heur;
  });
...
function solve_board_step(solve_state) {
  ...
  ++solve_state.p;
  solve_state.solutions = evolve_solutions(
...
$(document).ready(function () {
  ...
  $("#solutions").css(
    "min-height",
    $(window).height() - (navbar_height + second_navbar_height - 60),
  );
  $('[id^="grid"] > div').each(function () {
    $(this).addClass("eX");
  });
  ...
  $("#import-import").click(function () {
    ...
    var board = board_joined.match(boardre).map(function (s) {
      return s.split("");
    });
    show_board(board);
    clear_canvas();
