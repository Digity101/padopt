...
export default function Board(canvas_id, opts) {
  ...
  var _drawOrb = function (row, col, orb_index, is_dark_bg) {
    ...
    // draw background
    _ctx.drawImage(
      is_dark_bg ? _bg_img_dark : _bg_img_light,
  ...
  var _randomize = function (key_array) {
    ...
    for (var x = 0; x < _rows * _cols; x++) {
      _board_data.push(
        index_for_random[Math.floor(Math.random() * index_for_random.length)],
      );
    }
    _drawBoard();
  };
  ...
  var _drawPath = function (xys) {
    ...
    _ctx.beginPath();
    _ctx.rect(
      start_xy.x - _PATH_SQUARE_SIZE / 2,
      start_xy.y - _PATH_SQUARE_SIZE / 2,
      _PATH_SQUARE_SIZE,
      _PATH_SQUARE_SIZE,
    );
    _ctx.fill();
    ...
    _ctx.beginPath();
    _ctx.rect(
      end_xy.x - _PATH_SQUARE_SIZE / 2,
      end_xy.y - _PATH_SQUARE_SIZE / 2,
      _PATH_SQUARE_SIZE,
      _PATH_SQUARE_SIZE,
    );
    _ctx.fill();
  ...
  var _drawPathAnimation = function () {
    ...
    if (elapsed <= fps_interval) {
      ...
      _ctx.beginPath();
      _ctx.rect(
        _path_animation.x - _PATH_SQUARE_SIZE / 2,
        _path_animation.y - _PATH_SQUARE_SIZE / 2,
        _PATH_SQUARE_SIZE,
        _PATH_SQUARE_SIZE,
      );
      _ctx.fill();
      ...
      if (typeof target_xy != "undefined") {
        ...
        if (
          ...
          if (typeof next_target_xy != "undefined") {
            _path_animation.move_distance_x =
              (target_xy.x > next_target_xy.x
                ? target_xy.x - next_target_xy.x
                : next_target_xy.x - target_xy.x) /
              _NUMBER_OF_STEP_BETWEEN_TWO_POINT;
            _path_animation.move_distance_y =
              (target_xy.y > next_target_xy.y
                ? target_xy.y - next_target_xy.y
                : next_target_xy.y - target_xy.y) /
              _NUMBER_OF_STEP_BETWEEN_TWO_POINT;
          }
        } else {
          ...
          if (_path_animation.x !== target_xy.x) {
            _path_animation.x +=
              target_xy.x > _path_animation.x
                ? _path_animation.move_distance_x
                : _path_animation.move_distance_x * -1;
          }
          if (_path_animation.y !== target_xy.y) {
            _path_animation.y +=
              target_xy.y > _path_animation.y
                ? _path_animation.move_distance_y
                : _path_animation.move_distance_y * -1;
          }
        }
        _path_animation.request_id = _requestAnimationFrame(_drawPathAnimation);
      } else {
